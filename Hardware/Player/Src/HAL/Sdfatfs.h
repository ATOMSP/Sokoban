...
/*====Users config=====*/
#define SD_WORKMODE             (0)/*0: NO DMA 1: USE DMA*/
/*====Users config=====*/
...
/*	
*SDIO clock :SDIO_CK=SDIOCLK/[clkdiv+2];其中,SDIOCLK固定为48Mhz
*使用DMA模式的时候,传输速率可以到48Mhz(bypass on时),不过如果你的卡不是高速
*卡,可能也会出错,出错就请降低时钟
*/
#define SDIO_INIT_CLK_DIV        0x76 		//SDIO初始化频率，最大400Kh  
#define SDIO_TRANSFER_CLK_DIV    0x02		//SDIO传输频率,该值太小可能会导致读写文件出错 									 
...
public:
    ...
    SD_CardInfo SDCardInfo;//SD info
public:
...
private:
    ...
    uint8_t CardType;		//SD卡类型（默认为1.x卡）
    uint32_t CSD_Tab[4],CID_Tab[4],RCA;					//SD卡CSD,CID以及相对地址(RCA)数据
    uint8_t DeviceMode;		   				//工作模式,注意,工作模式必须通过SD_SetDeviceMode,后才算数.这里只是定义一个默认的模式(SD_DMA_MODE)
    uint8_t StopCondition; 								//是否发送停止传输标志位,DMA多块读写的时候用到  
