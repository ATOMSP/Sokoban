...
/*=================================SD相关操作:使用SD_Manager管理SD卡初始化提供文件系统读写PAI==========================================*/
...
SD_Manager::SD_Error SD_Manager::SD_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA2, ENABLE);//使能GPIOC,GPIOD DMA2时钟
	...
	{  		    
		...
		errorstatus=SD_SetDeviceMode(SD_WORKMODE);//设置工作模式为查询模式
 	}
	return errorstatus;		  
}
...
{	  
	...
	{
 		_INTX_DISABLE();//关闭总中断(POLLING模式,严禁中断打断SDIO读写操作!!!)
...
extern "C"{
    ...
    {		
        HAL::SD.SD_ProcessIRQSrc();//处理所有SDIO相关中断
    }	    
}
