...
#define	ABORT(fs, res)		{ fp->err = (_BYTE)(res); LEAVE_FF(fs, res); }
...
#define _DF1S	0x81	/* DBC 1st _BYTE range 1 start */
#define _DF1E	0x9F	/* DBC 1st _BYTE range 1 end */
#define _DF2S	0xE0	/* DBC 1st _BYTE range 2 start */
#define _DF2E	0xFC	/* DBC 1st _BYTE range 2 end */
#define _DS1S	0x40	/* DBC 2nd _BYTE range 1 start */
#define _DS1E	0x7E	/* DBC 2nd _BYTE range 1 end */
#define _DS2S	0x80	/* DBC 2nd _BYTE range 2 start */
#define _DS2E	0xFC	/* DBC 2nd _BYTE range 2 end */
...
#ifdef _DF2S	/* Two 1st _BYTE areas */
#define IsDBCS1(c)	(((_BYTE)(c) >= _DF1S && (_BYTE)(c) <= _DF1E) || ((_BYTE)(c) >= _DF2S && (_BYTE)(c) <= _DF2E))
#else			/* One 1st _BYTE area */
#define IsDBCS1(c)	((_BYTE)(c) >= _DF1S && (_BYTE)(c) <= _DF1E)
#endif

#ifdef _DS3S	/* Three 2nd _BYTE areas */
#define IsDBCS2(c)	(((_BYTE)(c) >= _DS1S && (_BYTE)(c) <= _DS1E) || ((_BYTE)(c) >= _DS2S && (_BYTE)(c) <= _DS2E) || ((_BYTE)(c) >= _DS3S && (_BYTE)(c) <= _DS3E))
#else			/* Two 2nd _BYTE areas */
#define IsDBCS2(c)	(((_BYTE)(c) >= _DS1S && (_BYTE)(c) <= _DS1E) || ((_BYTE)(c) >= _DS2S && (_BYTE)(c) <= _DS2E))
#endif
...
/* Name status flags */
#define NS			11		/* Index of name status _BYTE in fn[] */
...
/* FatFs refers the members in the FAT structures as _BYTE array instead of
/ structure member because the structure is not binary compatible between
/ different platforms */

#define BS_jmpBoot			0		/* Jump instruction (3) */
#define BS_OEMName			3		/* OEM name (8) */
#define BPB_BytsPerSec		11		/* Sector size [_BYTE] (2) */
...
#define	DEF_NAMEBUF			_BYTE sfn[12]
...
#define	DEF_NAMEBUF			_BYTE sfn[12]; WCHAR lbuf[_MAX_LFN+1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()

#elif _USE_LFN == 3 		/* LFN feature with dynamic working buffer on the heap */
#define	DEF_NAMEBUF			_BYTE sfn[12]; WCHAR *lfn
...
#ifdef _EXCVT
static
const _BYTE ExCvt[] = _EXCVT;	/* Upper conversion table for extended characters */
#endif
...
int mem_cmp (const void* dst, const void* src, UINT cnt) {
	const _BYTE *d = (const _BYTE *)dst, *s = (const _BYTE *)src;
	int r = 0;
...
)
{
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		case FS_FAT12 :
			...
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
			*p = (clst & 1) ? ((*p & 0x0F) | ((_BYTE)val << 4)) : (_BYTE)val;
			bc++;
			fs->wflag = 1;
			res = move_window(fs, fs->fatbase + (bc / SS(fs)));
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
			*p = (clst & 1) ? (_BYTE)(val >> 4) : ((*p & 0xF0) | ((_BYTE)(val >> 8) & 0x0F));
			break;
...
DWORD ld_clust (
	...
	_BYTE* dir	/* Pointer to the directory entry */
)
{
	DWORD cl;
...
#if _USE_LFN
static
const _BYTE LfnOfs[] = {1,3,5,7,9,14,16,18,20,22,24,28,30};	/* Offset of LFN characters in the directory entry */


static
int cmp_lfn (			/* 1:Matched, 0:Not matched */
	WCHAR* lfnbuf,		/* Pointer to the LFN to be compared */
	_BYTE* dir			/* Pointer to the directory entry containing a part of LFN */
)
{
	UINT i, s;
	WCHAR wc, uc;
...
static
void gen_numname (
	_BYTE* dst,			/* Pointer to the buffer to store numbered SFN */
	const _BYTE* src,	/* Pointer to SFN */
...
static
_BYTE sum_sfn (
	const _BYTE* dir		/* Pointer to the SFN entry */
)
{
	_BYTE sum = 0;
	UINT n = 11;
	...
	if (res == FR_OK && --nent) {	/* Set LFN entry if needed */
		...
		if (res == FR_OK) {
			...
			do {					/* Store LFN entries in bottom first */
				res = move_window(dp->fs, dp->sect);
				if (res != FR_OK) break;
				fit_lfn(dp->lfn, dp->dir, (_BYTE)nent, sum);
				dp->fs->wflag = 1;
				res = dir_next(dp, 0);	/* Next entry */
	...
	if (fno->lfname) {
		...
		if (dp->sect && fno->lfsize && dp->lfn_idx != 0xFFFF) {	/* Get LFN if available */
			...
			while ((w = *lfn++) != 0) {		/* Get an LFN character */
				...
				if (_DF1S && w >= 0x100)	/* Put 1st _BYTE if it is a DBC (always false on SBCS cfg) */
					p[i++] = (TCHAR)(w >> 8);
#endif
...
)
{
...
#if !_LFN_UNICODE
		w &= 0xFF;
		if (IsDBCS1(w)) {				/* Check if it is a DBC 1st _BYTE (always false on SBCS cfg) */
			b = (_BYTE)p[si++];			/* Get 2nd _BYTE */
			if (!IsDBCS2(b))
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	for (;;) {
		...
		if (_DF1S && w >= 0x100) {		/* Double _BYTE character (always false on SBCS cfg) */
			if (i >= ni - 1) {
				cf |= NS_LOSS | NS_LFN; i = ni; continue;
			}
			dp->fn[i++] = (_BYTE)(w >> 8);
		} else {						/* Single _BYTE character */
	...
	for (;;) {
		...
		if (IsDBCS1(c)) {				/* Check if it is a DBC 1st _BYTE (always false on SBCS cfg) */
			d = (_BYTE)p[si++];			/* Get 2nd _BYTE */
			if (!IsDBCS2(d) || i >= ni - 1)	/* Reject invalid DBC */
				return FR_INVALID_NAME;
			sfn[i++] = c;
			sfn[i++] = d;
		} else {						/* Single _BYTE code */
...
static
_BYTE check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
...
FRESULT find_volume (	/* FR_OK(0): successful, !=0: any error occurred */
	...
	_BYTE wmode			/* !=0: Check write protection for write access */
)
{
	_BYTE fmt;
	int vol;
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if (fmt == 1 || (!fmt && (LD2PT(vol)))) {	/* Not an FAT boot sector or forced partition number */
		...
		for (i = 0; i < 4; i++) {			/* Get partition offset */
			_BYTE *pt = fs->win+MBR_Table + i * SZ_PTE;
...
FRESULT f_mount (
	...
	_BYTE opt			/* 0:Do not mount (delayed mount), 1:Mount immediately */
)
{
	FATFS *cfs;
	int vol;
	...
	if (fs) {
		...
		if (!ff_cre_syncobj((_BYTE)vol, &fs->sobj)) return FR_INT_ERR;
#endif
	}
...
FRESULT f_open (
	...
	_BYTE mode			/* Access mode and file open mode flags */
)
{
	FRESULT res;
	DIR dj;
	...
	res = find_volume(&dj.fs, &path, (_BYTE)(mode & ~FA_READ));
#else
	mode &= FA_READ;
...
FRESULT f_read (
	...
	UINT btr,		/* Number of _BYTEs to read */
	UINT* br		/* Pointer to number of _BYTEs read */
)
{
	FRESULT res;
	DWORD clst, sect, remain;
	UINT rcnt, cc;
	_BYTE csect, *rbuff = (_BYTE*)buff;


	*br = 0;	/* Clear read _BYTE counter */
	...
	if (btr > remain) btr = (UINT)remain;		/* Truncate btr by remaining _BYTEs */

	for ( ;  btr;								/* Repeat until all data read */
		rbuff += rcnt, fp->fptr += rcnt, *br += rcnt, btr -= rcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			csect = (_BYTE)(fp->fptr / SS(fp->fs) & (fp->fs->csize - 1));	/* Sector offset in the cluster */
			...
			sect += csect;
			cc = btr / SS(fp->fs);				/* When remaining _BYTEs >= sector size, */
...
#endif
#endif
				rcnt = SS(fp->fs) * cc;			/* Number of _BYTEs transferred */
				continue;
			}
#if !_FS_TINY
...
FRESULT f_write (
	...
	UINT btw,			/* Number of _BYTEs to write */
	UINT* bw			/* Pointer to number of _BYTEs written */
)
{
	FRESULT res;
	DWORD clst, sect;
	UINT wcnt, cc;
	const _BYTE *wbuff = (const _BYTE*)buff;
	_BYTE csect;


	*bw = 0;	/* Clear write _BYTE counter */
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (!sect) ABORT(fp->fs, FR_INT_ERR);
			sect += csect;
			cc = btw / SS(fp->fs);			/* When remaining _BYTEs >= sector size, */
...
#endif
#endif
				wcnt = SS(fp->fs) * cc;		/* Number of _BYTEs transferred */
				continue;
			}
#if _FS_TINY
...
)
{
	...
	/* Normal Seek */
	{
		...
		fp->fptr = nsect = 0;
		if (ofs) {
			bcs = (DWORD)fp->fs->csize * SS(fp->fs);	/* Cluster size (_BYTE) */
			if (ifptr > 0 &&
...
FRESULT f_chmod (
	const TCHAR* path,	/* Pointer to the file path */
	_BYTE value,			/* Attribute bits */
	_BYTE mask			/* Attribute mask to change */
)
{
	FRESULT res;
	DIR dj;
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {						/* Is it a root directory? */
				...
				dir[DIR_Attr] = (value & mask) | (dir[DIR_Attr] & (_BYTE)~mask);	/* Apply attribute change */
				dj.fs->wflag = 1;
				res = sync_fs(dj.fs);
			}
		}
...
)
{
	...
	if (sl) {	/* Create volume label in directory form */
		...
		do {
			...
			w = (_BYTE)label[i++];
			if (IsDBCS1(w))
				w = (j < 10 && i < sl && IsDBCS2(label[i])) ? w << 8 | (_BYTE)label[i++] : 0;
#if _USE_LFN
...
FRESULT f_forward (
	FIL* fp, 						/* Pointer to the file object */
	UINT (*func)(const _BYTE*,UINT),	/* Pointer to the streaming function */
	UINT btf,						/* Number of _BYTEs to forward */
	UINT* bf						/* Pointer to number of _BYTEs forwarded */
)
{
	FRESULT res;
	...
	*bf = 0;	/* Clear transfer _BYTE counter */
	...
	remain = fp->fsize - fp->fptr;
	if (btf > remain) btf = (UINT)remain;			/* Truncate btf by remaining _BYTEs */
...
FRESULT f_mkfs (
	...
	_BYTE sfd,			/* Partitioning rule 0:FDISK, 1:SFD */
	UINT au				/* Allocation unit [_BYTEs] */
)
{
	static const WORD vst[] = { 1024,   512,  256,  128,   64,    32,   16,    8,    4,    2,   0};
	static const WORD cst[] = {32768, 16384, 8192, 4096, 2048, 16384, 8192, 4096, 2048, 1024, 512};
	int vol;
	_BYTE fmt, md, sys, *tbl, pdrv, part;
	...
	if (_MULTI_PARTITION && part) {
		...
		if (sfd) {	/* No partition table (SFD) */
			...
			tbl[6] = (_BYTE)(n >> 2 | 63);	/* Partition end sector */
			tbl[7] = (_BYTE)n;				/* End cylinder */
	...
	tbl[BPB_SecPerClus] = (_BYTE)au;			/* Sectors per cluster */
...
FRESULT f_fdisk (
	_BYTE pdrv,			/* Physical drive number */
	const DWORD szt[],	/* Pointer to the size table for each partitions */
	void* work			/* Pointer to the working buffer */
)
{
	UINT i, n, sz_cyl, tot_cyl, b_cyl, e_cyl, p_cyl;
	_BYTE s_hd, e_hd, *p, *buf = (_BYTE*)work;
	DSTATUS stat;
	...
	for (i = 0; i < 4; i++, p += SZ_PTE) {
		...
		/* Set partition table */
		p[1] = s_hd;						/* Start head */
		p[2] = (_BYTE)((b_cyl >> 2) + 1);	/* Start sector */
		p[3] = (_BYTE)b_cyl;					/* Start cylinder */
		p[4] = 0x06;						/* System type (temporary setting) */
		p[5] = e_hd;						/* End head */
		p[6] = (_BYTE)((e_cyl >> 2) + 63);	/* End sector */
		p[7] = (_BYTE)e_cyl;					/* End cylinder */
...
)
{
	...
	while (n < len - 1) {	/* Read characters until buffer gets filled */
		...
		if (c >= 0x80) {
			...
			if (c < 0xE0) {			/* Two-__BYTE sequence */
				f_read(fp, s, 1, &rc);
				if (rc != 1) break;
				c = (c & 0x1F) << 6 | (s[0] & 0x3F);
				if (c < 0x80) c = '?';
			} else {
				if (c < 0xF0) {		/* Three-__BYTE sequence */
					f_read(fp, s, 2, &rc);
					if (rc != 2) break;
					c = c << 12 | (s[0] & 0x3F) << 6 | (s[1] & 0x3F);
					if (c < 0x800) c = '?';
				} else {			/* Reject four-__BYTE sequence */
					c = '?';
				}
			}
		}
...
)
{
	...
	if (c < 0x80) {				/* 7-bit */
		...
		if (c < 0x800) {		/* 11-bit */
			pb->buf[i++] = (_BYTE)(0xC0 | c >> 6);
		} else {				/* 16-bit */
			pb->buf[i++] = (_BYTE)(0xE0 | c >> 12);
			pb->buf[i++] = (_BYTE)(0x80 | (c >> 6 & 0x3F));
		}
		pb->buf[i++] = (_BYTE)(0x80 | (c & 0x3F));
	}
