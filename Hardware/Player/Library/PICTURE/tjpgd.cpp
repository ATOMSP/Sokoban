...
static
const _BYTE Zig[64] = {	/* Zigzag-order to raster-order conversion table */
...
void* alloc_pool (	/* Pointer to allocated memory block (NULL:no memory available) */
	...
	UINT nd			/* Number of _BYTEs to allocate */
)
{
	char *rp = 0;
	...
	if (jd->sz_pool >= nd) {
		...
		jd->pool = (void*)(rp + nd);	/* Allocate requierd _BYTEs */
	}
...
UINT create_qt_tbl (	/* 0:OK, !0:Failed */
	JDEC* jd,			/* Pointer to the decompressor object */
	const _BYTE* data,	/* Pointer to the quantizer tables */
...
UINT create_huffman_tbl (	/* 0:OK, !0:Failed */
	JDEC* jd,				/* Pointer to the decompressor object */
	const _BYTE* data,		/* Pointer to the packed huffman tables */
	UINT ndata				/* Size of input data */
)
{
	...
	while (ndata) {	/* Process all tables in the segment */
		...
		pb = (_BYTE*)alloc_pool(jd, 16);			/* Allocate a memory block for the bit distribution table */
		if (!pb) return JDR_MEM1;			/* Err: not enough memory */
		...
		ndata -= np;
		pd = (_BYTE*)alloc_pool(jd, np);			/* Allocate a memory block for the decoded data */
...
)
{
	...
	do {
		if (!msk) {				/* Next _BYTE? */
			...
			dc--;				/* Decrement number of available _BYTEs */
...
INT huffext (			/* >=0: decoded data, <0: error code */
	JDEC* jd,			/* Pointer to the decompressor object */
	const _BYTE* hbits,	/* Pointer to the bit distribution table */
	const WORD* hcode,	/* Pointer to the code word table */
	const _BYTE* hdata	/* Pointer to the data table */
)
{
	_BYTE msk, s, *dp;
	UINT dc, v, f, bl, nd;
		...
		if (!msk) {		/* Next _BYTE? */
			...
			if (f) {		/* In flag sequence? */
				...
				*dp = s = 0xFF;			/* The flag is a data 0xFF */
				...
				s = *dp;				/* Get next data _BYTE */
				if (s == 0xFF) {		/* Is start of flag sequence? */
					f = 1; continue;	/* Enter flag sequence, get trailing _BYTE */
				}
			}
			msk = 0x80;		/* Read from MSB */
		}
...
void block_idct (
	...
	_BYTE* dst	/* Pointer to the destination to store the block as _BYTE array */
)
{
...
)
{
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			a = (mx - w) * 3;	/* _BYTEs to skip for next line in the square */
			op = (_BYTE*)jd->workbuf;
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					rgb24 = (_BYTE*)jd->workbuf + (iy * mx + ix) * 3;
					r = g = b = 0;
...
)
{
	...
	/* Discard padding bits and get two _BYTEs from the input stream */
...
#define	LDB_WORD(ptr)		(WORD)(((WORD)*((_BYTE*)(ptr))<<8)|(WORD)*(_BYTE*)((ptr)+1))


JRESULT jd_prepare (
	JDEC* jd,			/* Blank decompressor object */
	UINT (*infunc)(JDEC*, _BYTE*, UINT),	/* JPEG strem input function */
...
)
{
	...
	jd->inbuf = seg = (_BYTE*)alloc_pool(jd, JD_SZBUF);		/* Allocate stream input buffer */
	...
	for (;;) {
		...
		ofs += 4 + len;	/* Number of _BYTEs loaded */
		...
		case 0xDA:	/* SOS */
			...
			if (len < 256) len = 256;					/* but at least 256 _BYTE is required for IDCT */
			jd->workbuf = alloc_pool(jd, len);			/* and it may occupy a part of following MCU working buffer for RGB output */
			if (!jd->workbuf) return JDR_MEM1;			/* Err: not enough memory */
			jd->mcubuf = (_BYTE*)alloc_pool(jd, (n + 2) * 64);	/* Allocate MCU working buffer */
		...
		default:	/* Unknown segment (comment, exif or etc..) */
			/* Skip segment data */
			if (jd->infunc(jd, 0, len) != len)	/* Null pointer specifies to skip _BYTEs of stream */
				return JDR_INP;
		}
	}
}
...
JRESULT jd_decomp (
	...
	UINT (*outfunc)(JDEC*, void*, JRECT*),	/* RGB output function */
	_BYTE scale								/* Output de-scaling factor (0 to 3) */
)
{
	UINT x, y, mx, my;
	WORD rst, rsc;
